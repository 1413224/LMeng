<template>
	<div class="font-class-box">
		<ul ref="icon_lists" class="icon_lists clear">
			...
			<li>
				<i class="icon iconfont icon-xiaoheiqun"></i>
				<div class="name">xiaoheiqun</div>
				<div class="fontclass">.icon-xiaoheiqun</div>
			</li>

			<li>
				<i class="icon iconfont icon-auction"></i>
				<div class="name">auction</div>
				<div class="fontclass">.icon-auction</div>
			</li>
...
<script>
	export default {
		...
		mounted() {
			let vm = this
			for(var i = 0; i < document.querySelectorAll('.icon_lists li').length; i++) {
				document.querySelectorAll('.icon_lists li')[i].addEventListener('click', function(e) {
					if(event.target == this) {
						vm.$emit('setData', e.target.firstElementChild.className)
					}else{
						vm.$emit('setData', e.target.className)
					}
				})
			}
		}
