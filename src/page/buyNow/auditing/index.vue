            ...
            <el-table :data="goodsData" @selection-change="handleSelectionChange" style="width: 100%">
				...
				<el-table-column label="操作" min-width="90" fixed="right">
                    <template slot-scope="scope">
                        ...
                        <el-tooltip class="item" effect="dark" content="删除" v-has="'del-auditing'" placement="top">
                            <el-button
                            size="mini"
                            type="danger"
                            icon="el-icon-delete"
...
export default {
	...
	methods: {
		...
		getGoodsList () {
			...
			vm.$get(vm.$api.buy.getGoodsCheckList, param).then(resp => {
		...
		goDetail (id, type) {
            let path = type == 2 ? 'buy-now-contrast' : 'buy-now-auditing-edit'+this.type
        ...
        delCheckGoodsById (id) {
			...
			vm.$post(vm.$api.buy.delCheckGoodsById, param).then(resp => {
                vm.$message({
                    type: 'success',
                    message: '操作成功!'
                })
