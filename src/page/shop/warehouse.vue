	...
	<div class="income">
		...
		<el-card class="box-card">
            ...
            <div style="padding: 10px;">
                <el-button type="danger" size="small" @click="openConfirm(goodsIds, '是否将选中商品删除?', delGoodsByIds)" v-has="'del-warehouse'" :disabled="!goodsIds">批量删除</el-button>
            </div>
            <el-table :data="goodsData" @selection-change="handleSelectionChange" style="width: 100%">
                ...
                <el-table-column prop="status" label="状态" min-width="70">
                    <template slot-scope="scope">
                		<el-tag id="up-warehouse" v-has="'up-warehouse'" size="mini" v-show="scope.row.total" type="warning" @click.native="openConfirm(scope.row.goodsId, '是否将此商品上架?', changeGoodsStatusById)" class="mouse-hand">已下架</el-tag>
                		<el-tag size="mini" v-if="upWarehouse && scope.row.total" type="warning">已下架</el-tag>
                    	<el-tag size="mini" v-if="!scope.row.total" type="info">已售罄</el-tag>
                    </template>
                </el-table-column>
				...
				<el-table-column label="操作" min-width="90" fixed="right">
                    <template slot-scope="scope">
                        <el-tooltip class="item" effect="dark" content="编辑" v-has="'warehouse-detail'" placement="top">
                            <el-button
                            size="mini"
                            type="primary"
                        ...
                        <el-tooltip class="item" effect="dark" content="删除" v-has="'del-warehouse'" placement="top">
                            <el-button
                            size="mini"
                            type="danger"
                            icon="el-icon-delete"
...
export default {
	...
	updated () {
		this.upWarehouse = !document.querySelector('#up-warehouse')
	},
	methods: {
		resetting () {
			let vm = this
		...
		goDetail (id) {
			this.$router.push({ path: 'warehouse-detail', query: { gid: id }})
        },
