                    ...
                    <el-row>
                        <el-form label-width="140px" :model="alliance" :rules="allianceRules">
                            ...
                            <el-form-item label="退货时间 :" prop="returnDeadline">
                                <span v-if="!alliance.returnDeadlineType">{{Common.returnDeadline | formatSecs}}</span>
                                <template v-else>
            	...
            	<el-card class="goods-card">
                    ...
                    <el-row class="goods-row long-span">
                        <el-form style="max-width: 800px;">
                            ...
                            <!-- <div class="flex-center">
                                <el-button type="primary" @click="editGoodsParamsInfoById">保 存</el-button>
                            </div> -->
                        </el-form>
                    </el-row>
                </el-card>
    ...
    data () {
        ...
        return {
            ...
            allianceRules: {
                ...
                returnDeadlineType: [{ required: true, message: '请选择退货时间方式', trigger: 'change' }],
                taxRateType: [{ required: true, message: '请选择税率方式', trigger: 'change' }],
                serviceRateType: [{ required: true, message: '请选择服务费率方式', trigger: 'change' }],
                returnDeadline: [{ required: true, validator: validateReturnDeadline, trigger: 'blur' }],
    ...
    methods: {
        ...
        openBaseVisible () {
            ...
            vm.base = {
                ...
                originAreaParentIds: vm.info.originAreaInfo ? vm.info.originAreaInfo.map(obj => obj.areaId) : [],
        ...
        editGoodsParamsInfoById () {
            ...
            let param = {
                accountId: localStorage['userId'+sessionStorage['roleTypeId']],
                goodsId: vm.gid,
                params: JSON.stringify(vm.goodsParam.map((obj, index) => {
        ...
        editGoodsInfoById () {
            ...
            vm.$refs['dateForm'].validate((valid) => {
                if (valid) {
                    ...
                    vm.$post(vm.$api.shop.editGoodsInfoById, param).then(resp => {
                        vm.$message({
                            message: '保存成功',
                            type: 'success'
                        })
        ...
        correcting () {
            ...
            vm.$refs['baseForm'].validate((valid) => {
                if (valid) {
                    if (vm.alliance.isInvoice==vm.info.isInvoice&&vm.alliance.returnDeadlineType==vm.info.returnDeadlineType&&vm.alliance.returnDeadline==vm.formatDead(vm.info.returnDeadline)&&vm.alliance.taxRateType==vm.info.taxRateType&&vm.alliance.taxRate==vm.info.taxRate&&vm.alliance.serviceRateType==vm.info.serviceRateType&&vm.alliance.serviceRate==vm.info.serviceRate) {
                        if (vm.info.status == 1) {
        ...
        goAuditing (id) {
            this.$router.push({ path: 'shop-auditing-contrast', query: { gid: id }})
        },
        getCommonSetting () {
            let vm = this
            let param = {
            ...
            vm.$get(vm.$api.setting.getCommonSetting, param).then(resp => {
                vm.Common.returnDeadline = resp.returnDeadline
