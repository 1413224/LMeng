			...
			<el-row class="base-cont">
              ...
              <img v-else src="../../../assets/images/goods_default.png" alt="" class="image">
							<div class="tit">
								...
								<p class="tit_price">
									<span>销售价 ：</span><span>{{info.price}}</span>
									...
									<span style="text-decoration: line-through;margin-left: 15px;color: #909399;font-size: 14px;font-weight: 400;">
										...
										￥{{info.minOriginPrice}}<span>{{info.originPrice}}</span>
									</span>
									
								</p>
								<!-- <p v-if="info.minIncome==info.maxIncome" style="color: #ff343d;font-weight: 700;">预计收益：￥{{info.minIncome}}</p> -->
                <p  style="color: #ff343d;font-weight: 700;">预计收益 :￥{{info.income}}</p>
							...
							<div class="tit_concern" >
								商品库存<br/>
								<span style="color: #F23030;">{{info.stock}}</span>
							</div>
	...
	export default {
		...
		methods: {
			getSupplyGoodsDetail() {
				...
				vm.$get(vm.$api.money.getSupplyGoodsDetail, param).then(resp => {
          vm.info = resp
					vm.spec = {
		...
		created() {
			...
			if(this.gid) {
				this.getSupplyGoodsDetail(this.gid,this.skuId)
			} else {
				this.$message({
					type: 'error',
					message: '错误的访问'
