    	    ...
    	    <div class="enterprise-info-content" v-if="info.enterpriseInfo">
    	      <el-row>
    	        <el-col :span="24">
                  ...
                  <!-- <img style="width:80px;height:80px;border: 1px solid #e1e7f5;" src="../../../assets/images/goods_default.png" alt=""> -->
	...
	export default {
		...
		methods: {
			getOnlineGoodsDetail() {
				...
				vm.$get(vm.$api.money.getOnlineGoodsDetail, param).then(resp => {
          vm.info = resp
          console.log(vm.info.enterpriseInfo.logo)
					vm.spec = {
		...
		created() {
			...
			if(this.gid) {
				this.getOnlineGoodsDetail(this.gid)
			} else {
				this.$message({
					type: 'error',
					message: '错误的访问'
