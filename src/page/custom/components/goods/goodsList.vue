<template>
	<div class="goods">
		<el-card class="box-card">
			...
			<!-- 表格数据 -->
			<div class="border">
				<el-table :data="goodsData" maxHeight="404" style="width: 100%" @selection-change="handleSelectionChange">
	...
	export default {
		...
		methods: {
			...
			setList() {
				...
				this.$emit('getGoodsList', this.list)
			},
			resetting() {
				this.keyword = ''
				this.search = {
			...
			getGoodsList() {
				let vm = this
				if(vm.goodsSource.length == 1) {
					if(vm.goodsSource[0] == 'hasShopGoods') {
						var hasShopGoods = 1
					} else {
						var hasSupplyGoods = 1
					}
				} else if(vm.goodsSource.length == 2) {
					var hasShopGoods = 1
					var hasSupplyGoods = 1
				}
				let param = Object.assign({}, vm.search, {
					...
					hasShopGoods: hasShopGoods,
					hasSupplyGoods: hasSupplyGoods
				})
